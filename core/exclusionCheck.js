...
export async function isExcluded(dealId, contact) {
  ...
  for (const ex of exclusions) {
    // Exact email match always applies
    if (ex.email && emailNorm && emailNorm === ex.email) return true;

    // Exact person name match always applies
    if (ex.person_name && personNorm && personNorm === ex.person_name) return true;

    // Firm name match ONLY for institutional contacts — not angels
    // Angels should not be excluded because their name resembles a firm entry
    if (!isAngel && ex.firm_name && firmNorm) {
